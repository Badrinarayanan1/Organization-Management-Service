...
async def rename_and_sync_collection(old_org_name: str, new_org_name: str):
    ...
    if old_coll_name not in collections:
        # If it doesn't exist, just ensure the new one is "ready" (lazy creation)
        return True

    # 1. Create new collection (to ensure it exists before copy)
    if new_coll_name not in collections:
        await db.create_collection(new_coll_name)
    ...
    # Using efficient cursor iteration
